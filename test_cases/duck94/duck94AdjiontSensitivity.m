...
% the analysis.  This defines which case you want to test.  Do a full model
% run to regenerate the time-dependent background.
load case_b_outputs/assimIter1/output.mat
modelinput=initModelInputs(duck94Case,grid,bkgd_1.sedmodel);
if(length(hydroobs)~=bathyobs(end).obsn)
  error('bathyobs time does not coincide with end of model period.  Didn''t plan for this, code will need tweaking');
end
