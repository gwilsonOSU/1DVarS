function [theta_av,ksd,ksw,fd,fw,...
    ...
    vanderA_shields_ja(d50,d90,udelta,uhat,delta,mu,eta,lambda,ahat)
%
...
% hack: if lambda=eta=0, then just set lambda=1 and eta=0.  The point is
% just to ignore ripples if they don't occur.  Will need to be careful about
% this in TL coding.
if(lambda==0 & eta==0)
  lambda=1;
  eta=0;
...
kvecguess=[1; 1]*.001;
kvec = fsolve(@(kvec)[ksd((kvec(1)),(kvec(2)))-(kvec(1));
                      ksw((kvec(1)),(kvec(2)))-(kvec(2))],...
              kvecguess,opt);
