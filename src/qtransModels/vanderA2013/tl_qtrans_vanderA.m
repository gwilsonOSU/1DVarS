...
elseif(~isfield(param,'nosusp') | param.nosusp==0)  % OPTION-2
  ...
  arg_Kidenom1=utot.^3;
  tl_arg_Kidenom1 = 3*utot.^2.*tl_utot;
  % Kidenom1 = + eps_s/ws*mean(arg_Kidenom1);
  tl_Kidenom1 = ...
      - eps_s/ws^2*mean(arg_Kidenom1)*tl_ws ...
      + eps_s/ws*mean(tl_arg_Kidenom1);
  arg_Kidenom2 = utot.^4;
  tl_arg_Kidenom2 = 4*utot.^3.*tl_utot;
  % Kidenom2 = - (eps_s/ws)^2*mean(arg_Kidenom2)*tanbeta;
  tl_Kidenom2 = ...
  ...
  % Ki = Omegai*(s-1)*g*d50/(Kidenom1+Kidenom2);  % chosen s.t. EM-model predicts sediment load = Omegai
  tl_Ki = ...
  ...
  tl_bedSuspRatio = (tl_Kbdenom1 + tl_Kbdenom2)/(Ksdenom1 + Ksdenom2) ...
