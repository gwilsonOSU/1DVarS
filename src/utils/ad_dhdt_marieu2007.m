...
for j=i_interp(:)'
  ...
  tl_a_orig(ind) = tl_a_orig(ind) + Xi'*ad_pa;
  ad_pa=0;
end
warning on
...
%2 add ghost points
if(ghost==+1)
  %2b1 tl_h=[tl_hin; tl_hin(nx)];
  ad_hin(1:nx) = ad_hin(1:nx) + ad_h(1:nx);
  ad_hin(nx) = ad_hin(nx) + ad_h(nx+1);
  ad_h=0;
  %2a1 tl_q=[tl_qin; tl_qin(nx)];
  ad_qin(1:nx) = ad_qin(1:nx) + ad_q(1:nx);
  ad_qin(nx) = ad_qin(nx) + ad_q(nx+1);
  ad_q=0;
else
  %2b2 tl_h=[tl_hin(1); tl_hin];
  ad_hin(1) = ad_hin(1) + ad_h(1);
  ad_hin(1:nx) = ad_hin(1:nx) + ad_h(2:nx);
  ad_h=0;
  %2a2 tl_q=[tl_qin(1); tl_qin];
  ad_qin(1) = ad_qin(1) + ad_q(1);
  ad_qin(1:nx) = ad_qin(1:nx) + ad_q(2:nx);
  ad_q=0;
end
